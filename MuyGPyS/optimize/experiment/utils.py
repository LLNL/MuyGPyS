...
class UnivariateSampler:
    def __init__(
        ...
        kernel=Matern(
            ...
            metric=IsotropicDistortion("l2", length_scale=ScalarHyperparameter(1.0)),
        ),
        ...
        self.test_features = self.x[np.mod(np.arange(data_count), train_step) != 0, :]
    ...
    def plot_results(
        ...
        _, axes = plt.subplots(2, 1, figsize=(15, 16))

        self.plot_target(axes[0])
        self.plot_model(axes[0], "Bayes", bayes_predictions, bayes_confidence_intervals)
        self.plot_target_sub(axes[1])
        self.plot_model_sub(
            axes[1], "Bayes", bayes_predictions, bayes_confidence_intervals
        )
    ...
    def plot_target(self, ax):
        ...
        ax.plot(self.test_features, self.test_responses, "g-", label="test response")
    ...
    def plot_target_sub(self, ax):
        ...
        ax.plot(
            self.test_features[
                self.mid * (self.train_step - 1) : self.mid * (self.train_step - 1)
    ...
    def plot_model_sub(self, ax, name, predictions, confidence_intervals):
        confidence_intervals = confidence_intervals.reshape(self.test_count)
        ax.set_title(f"Sampled Curve (subset) with {name} optimized model", fontsize=24)
        ax.plot(
            self.test_features[
                self.mid * (self.train_step - 1) : self.mid * (self.train_step - 1)
                + (self.vis_subset_size * (self.train_step - 1))
...
def print_results(name, targets, means, variances, confidence_intervals, coverage):
