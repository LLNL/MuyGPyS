...
class MuyGPS:
    ...
    Args:
        kernel:
            The kernel to be used. Defines :math:`Kcov_\\theta(\\cdot, \\cdot)` as
    ...
    def posterior_mean(
        self, Kcov: mm.ndarray, Kcross: mm.ndarray, batch_nn_targets: mm.ndarray
    ) -> mm.ndarray:
        """
        ...
        the last two dimensions of `Kcov` along with `Kcross` and
        `batch_nn_targets` to predict responses in terms of the posterior mean.
        Assumes that kernel tensor `Kcov` and cross-covariance
        ...
        :math:`Kcov_\\theta(\\cdot, \\cdot)`, computes the following for each
        prediction element :math:`\\mathbf{z}_i` with nearest neighbors index
        set :math:`N_i`:

        .. math::
            \\widehat{Y} (\\mathbf{z}_i \\mid X_{N_i}) =
                \\sigma^2 Kcov_\\theta (\\mathbf{z}_i, X_{N_i})
                (Kcov_\\theta (X_{N_i}, X_{N_i}) + \\varepsilon_{N_i})^{-1}
                Y(X_{N_i}).
        ...
        """
        return self._mean_fn(Kcov, Kcross, batch_nn_targets)
    ...
    def posterior_variance(
        ...
        .. math::
            ...
            \\right) =
                \\sigma^2 \\left (
                    Kcov_\\theta (\\mathbf{z}_i, \\mathbf{z}_i) -
                    Kcov_\\theta (\\mathbf{z}_i, X_{N_i})
                    \\left (
                        Kcov_\\theta (X_{N_i}, X_{N_i}
                    \\right ) + \\varepsilon_{N_i})^{-1}
                    Kcov_\\theta (X_{N_i}, \\mathbf{z}_i)
                \\right ).
        ...
        """
        return self._var_fn(Kcov, Kcross)
    ...
    def fast_coefficients(
        ...
        :math:`Kcov_\\theta(\\cdot, \\cdot)`, computes the following for
        ...
        .. math::
            C_i =
                \\left ( Kcov_\\theta (X_{N_i}, X_{N_i} \\right )
        ...
        return self._fast_precompute_fn(Kcov, train_nn_targets_fast)
    ...
    def fast_posterior_mean(
        ...
        :math:`Kcov_\\theta(\\cdot, \\cdot)`, computes the following for each test
        ...
        .. math::
            \\widehat{Y} \\left ( \\mathbf{z} \\mid X \\right ) =
                \\sigma^2 Kcov_\\theta(\\mathbf{z}, X_{N^*_i}) C_i.
    ...
    def optimize_scale(
        ...
        Returns:
            ...
            parameter.
        """
        Kcov = self.kernel(pairwise_diffs)
        opt_fn = self.scale.get_opt_fn(self)
        self.scale._set(opt_fn(Kcov, nn_targets))
        self._make()
        return self
