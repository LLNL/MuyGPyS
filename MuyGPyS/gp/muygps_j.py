...
from MuyGPyS.gp.kernels_j import (
    _get_kernel,
...
class MuyGPS:
    ...
    def _compute_solve_n(
        ...
        for i in range(batch_count):
            responses[i, :] = Kcross[i, :] @ np.linalg.solve(
                K[i, :, :] + eps * np.eye(nn_count), batch_nn_targets[i, :, :]
            )
        return responses.reshape(batch_count, response_count)
    ...
    def get_opt_fn(self):
        ...
        Returns:
            ...
            in :func:`~MuyGPyS.gp.muygps.MuyGPS.get_optim_params()`.
        """
        if not self.eps.fixed():
