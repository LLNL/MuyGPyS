...
class HomoscedasticNoise(ScalarParam, NoiseFn):
    ...
    def perturb(
        self, Kcov: mm.ndarray, noise: Optional[float] = None, **kwargs
    ) -> mm.ndarray:
        """
        ...
        submatrix :math:`Kcov`, computes the form :math:`Kcov + \\tau^2 * I`,
        ...
        if noise is None:
            noise = self._val
        return self._perturb_fn(Kcov, noise)
    ...
    def perturb_fn(self, fn: Callable) -> Callable:
        ...
        def perturbed_fn(Kcov, *args, noise=None, **kwargs):
            return fn(self.perturb(Kcov, noise=noise), *args, **kwargs)
