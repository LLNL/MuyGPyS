...
class MultivariateMuyGPS:
    ...
    Example:
        ...
        >>>     Kcov = model.kernel(pairwise_diffs)
        >>>     Kcross = model.kernel(crosswise_diffs)
        >>>     # do something with Kcov and Kcross...
    ...
    def posterior_mean(
        ...
        kernel function :math:`Kcov_{\\theta^{(j)}}(\\cdot, \\cdot)`, computes the
        ...
        .. math::
            \\widehat{Y} (\\mathbf{z}_i \\mid X_{N_i})_j =
                \\sigma^2_j Kcov_{\\theta^{(j)}} (\\mathbf{z}_i, X_{N_i})
                \\left (
                    Kcov_{\\theta^{(j)}} (X_{N_i}, X_{N_i})
                    + \\varepsilon^{(j)}_{N_i}
                \\right )^{-1}
                Y(X_{N_i})_{:,j}.
    ...
    def posterior_variance(
        ...
        :math:`Kcov_{\\theta^{(j)}}(\\cdot, \\cdot)`, computes the following for
        ...
        .. math::
            ...
            \\right)_j =
                \\sigma_j^2 \\left (
                    Kcov_{\\theta^{(j)}} (\\mathbf{z}_i, \\mathbf{z}_i) -
                    Kcov_{\\theta^{(j)}} (\\mathbf{z}_i, X_{N_i})
                    \\left (
                        Kcov_{\\theta^{(j)}} (X_{N_i}, X_{N_i}
                    \\right ) + \\varepsilon^{(j)}_{N_i})^{-1}
                    Kcov_{\\theta^{(j)}} (X_{N_i}, \\mathbf{z}_i)
                \\right ).
        ...
        for i, model in enumerate(self.models):
            ...
            diagonal_variance = mm.assign(
                diagonal_variance,
                model.posterior_variance(Kcov, Kcross).reshape(batch_count) * ss,
                slice(None),
                i,
            )
    ...
    def fast_coefficients(
        ...
        for i, model in enumerate(self.models):
            Kcov = model.kernel(pairwise_diffs_fast)
            mm.assign(
                coeffs_tensor,
                model.fast_coefficients(
                    model.noise.perturb(Kcov),
    ...
    def fast_posterior_mean(
        ...
        :math:`Kcov_{\\theta^{(j)}}(\\cdot, \\cdot)`, computes the following for each
        ...
        .. math::
            \\widehat{Y} \\left ( \\mathbf{z} \\mid X \\right )_j =
                \\sigma^2 Kcov_{\\theta^{(j)}}(\\mathbf{z}, X_{N^*_i}) C^{(j)}_i.
