...
"""
Copy of JAX config object.

This code is included so that MuyGPySConfig can inherit from JaxConfig if the
JAX dependency is not included.
"""
...
class Config:
    ...
    def DEFINE_bool(self, name, default, *args, **kwargs):
        ...
        self.add_option(
            name, default, bool, args, kwargs, update_hook=update_hook
        )

    def DEFINE_integer(self, name, default, *args, **kwargs):
        update_hook = kwargs.pop("update_hook", None)
        self.add_option(
            name, default, int, args, kwargs, update_hook=update_hook
        )

    def DEFINE_string(self, name, default, *args, **kwargs):
        update_hook = kwargs.pop("update_hook", None)
        self.add_option(
            name, default, str, args, kwargs, update_hook=update_hook
        )
    ...
    def __call__(self, new_val=no_default):
        if new_val is no_default:
            if self._default_value is not no_default:
                new_val = (
                    self._default_value
                )  # default_value provided to constructor
            else:
...
#     help=(
#         "Deprecated. Setting this flag to False raises an error. Setting it "
...
#         '("allow", "warn", or "raise").'
#     ),
#     update_global_hook=lambda val: update_global_jit_state(
...
#         "dynamic shapes."
#     ),
#     update_global_hook=lambda val: update_global_jit_state(dynamic_shapes=val),
#     update_thread_local_hook=lambda val: update_thread_local_jit_state(
#         dynamic_shapes=val
#     ),
# )
