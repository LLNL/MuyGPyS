...
def _rbf_fn(
    squared_dists: torch.ndarray, length_scale: float, **kwargs
) -> torch.ndarray:
    return torch.exp(-squared_dists / (2 * length_scale**2))


def _matern_05_fn(
    dists: torch.ndarray, length_scale: float, **kwargs
) -> torch.ndarray:
...
def _matern_gen_fn(
    dists: torch.ndarray, nu: float, length_scale: float, **kwargs
) -> torch.ndarray:
