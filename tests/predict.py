...
from MuyGPyS.old_predict import (
    classify_any as old_classify_any,
    classify_two_class_uq as old_classify_two_class_uq,
)
...
from MuyGPyS.old_uq import (
    train_two_class_interval as old_train_two_class_interval,
)
...
class ClassifyTest(parameterized.TestCase):
    @parameterized.parameters(
        (
            (1000, 200, f, r, nn, nn_kwargs, e, k_kwargs)
            for f in [100, 10, 2]
            for r in [10, 2, 1]
            for nn in [5, 10, 100]
            ...
            for k_kwargs in (
                (
                    "matern",
                    {"nu": {"val": 0.38}, "length_scale": {"val": 1.5}},
                ),
            )
        )
    )
    ...
    @parameterized.parameters(
        (
            (1000, 200, f, r, nn, b, nn_kwargs, e, k_kwargs)
            # for f in [100]
            # for r in [2]
            # for nn in [10]
            # for b in [200]
...
class OldClassifyTest(parameterized.TestCase):
    @parameterized.parameters(
        (
    ...
    def test_old_classify_any(
        ...
        predictions, _ = old_classify_any(
            muygps,
            test["input"],
            train["input"],
            nbrs_lookup,
    ...
    def test_classify_uq(
        ...
        predictions, variances, _ = old_classify_two_class_uq(
            muygps,
            test["input"],
            train["input"],
            nbrs_lookup,
        ...
        cutoffs = old_train_two_class_interval(
            muygps,
            indices,
            nn_indices,
            train["input"],
