...
class GPTestCase(parameterized.TestCase):
    def _prepare_tensors(
        ...
        # make kernels
        Kcov = _consistent_unchunk_tensor(muygps.kernel(pairwise_diffs))
        Kcross = _consistent_unchunk_tensor(muygps.kernel(crosswise_diffs))
        _check_ndarray(self.assertEqual, Kcov, mm.ftype)
        _check_ndarray(self.assertEqual, Kcross, mm.ftype)
        # do validation
        self.assertEqual(Kcov.shape, (test_count, nn_count, nn_count))
...
class GPTensorShapesTest(GPTestCase):
    ...
    def test_tensor_shapes(
        ...
        Kcov, Kcross, _, _, _, _ = self._prepare_tensors(
            muygps,
            train_count,
            test_count,
            feature_count,
        ...
        self.assertTrue(mm.all(Kcov >= 0.0))
        self.assertTrue(mm.all(Kcov <= 1.0))
        ...
        if config.state.low_precision() is True:
            ...
            return
        for i in range(Kcov.shape[0]):
            eigvals = mm.linalg.eigvals(Kcov[i, :, :])
...
class HomoscedasticNoiseTest(GPTestCase):
    ...
    def test_homoscedastic_perturb(
        ...
        perturbed_Kcov = _homoscedastic_perturb(Kcov, muygps.noise())
        _check_ndarray(self.assertEqual, perturbed_Kcov, mm.ftype)

        manual_Kcov = Kcov + muygps.noise() * mm.eye(nn_count)
        self.assertTrue(mm.allclose(perturbed_Kcov, manual_Kcov))
        return perturbed_Kcov, Kcross
...
class HeteroscedasticNoiseTest(GPTestCase):
    ...
    def test_heteroscedastic_perturb(
        ...
        perturbed_Kcov = _heteroscedastic_perturb(Kcov, muygps.noise())
        _check_ndarray(self.assertEqual, perturbed_Kcov, mm.ftype)

        manual_Kcov = Kcov + noise_tensor
        self.assertTrue(mm.allclose(perturbed_Kcov, manual_Kcov))
        return perturbed_Kcov, Kcross
...
class GPSolveTest(GPTestCase):
    ...
    def test_tensor_solve(
        ...
        responses = _consistent_unchunk_tensor(
            muygps.posterior_mean(Kcov, Kcross, batch_nn_targets)
        )
        ...
        for i in range(test_count):
            manual_responses = Kcross[i, :] @ mm.linalg.solve(
                Kcov[i, :, :] + muygps.noise() * mm.eye(nn_count),
...
class GPDiagonalVariance(GPTestCase):
    ...
    def test_diagonal_variance(
        ...
        diagonal_variance = _consistent_unchunk_tensor(
            muygps.posterior_variance(Kcov, Kcross)
        )
...
class GPScaleTest(GPTestCase):
    ...
    def test_batch_scale_shapes(
        ...
        Kcov = _consistent_unchunk_tensor(Kcov)
