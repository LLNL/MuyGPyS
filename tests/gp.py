...
class GPInitTest(parameterized.TestCase):
    ...
    def test_full_init(self, kern, hyper_dict):
        ...
        for key in hyper_dict:
            if key == "eps":
                self.assertEqual(hyper_dict[key], lkgp.eps)
            elif key == "sigma_sq":
                self.assertEqual(hyper_dict[key], lkgp.sigma_sq)
            else:
                self.assertEqual(hyper_dict[key], lkgp.params[key])
...
class GPMathTest(parameterized.TestCase):
    ...
    def test_tensor_shapes(
        ...
        K, Kcross = lkgp._compute_kernel_tensors(
    ...
    def test_tensor_solve(
        ...
        solve = lkgp._compute_solve(nn_indices, train["output"], K, Kcross)
    ...
    def test_diagonal_variance(
        ...
        diagonal_variance = lkgp._compute_diagonal_variance(
        ...
        for i in range(test_count):
            self.assertAlmostEqual(
                ...
                - Kcross[i, 0, :]
                @ np.linalg.solve(
                    K[i, :, :] + lkgp.eps * np.eye(nn_count), Kcross[i, 0, :]
                ),
            )
...
class GPSigmaSqTest(parameterized.TestCase):
    ...
    def test_batch_sigma_sq_shapes(
        ...
        lkgp.sigma_sq_optim(indices, nn_indices, data["input"], data["output"])
        self.assertEqual(lkgp.sigma_sq.shape, (response_count,))
        for i in range(response_count):
            sigmas = lkgp.get_sigma_sq(
                indices, nn_indices, data["input"], data["output"][:, i]
            )
            self.assertEqual(sigmas.shape, (data_count,))
            self.assertAlmostEqual(lkgp.sigma_sq[i], np.mean(sigmas), 5)
        # print(sigmas.shape)
