...
from MuyGPyS.testing.old_api_tests import ClassifyAPITest, RegressionAPITest
...
class MNISTTest(ClassifyAPITest):
    ...
    def test_classify_notrain_noembed(
        ...
        self.old_do_classify_test_chassis(
            train=train,
            test=test,
    ...
    def test_classify_noembed(
        ...
        train = balanced_subsample(self.embedded_40_train, 1000)
        test = balanced_subsample(self.embedded_40_test, 1000)
        self._old_do_classify_test_chassis(
            train=train,
            test=test,
            target_acc=target_accuracy,
            nn_count=nn_count,
...
class HeatonTest(RegressionAPITest):
    ...
    def test_regress_notrain(
        ...
        self._old_do_regress_test_chassis(
            train=self.train,
            test=self.test,
